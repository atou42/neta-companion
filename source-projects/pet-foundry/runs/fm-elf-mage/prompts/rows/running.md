Create a single horizontal sprite strip for the Codex app digital pet `fm-silver-elf-mage` in the state `running`.
...
Animation action: FM switching-track loop, the mage performs a quick in-place step and tiny teleport-ready turn, as if catching the next signal.


State-specific requirements:
- Show track switching through a compact in-place action: one foot shifts, cloak turns, staff tucks in, body leans forward, then resets.
- The character must stay centered in each slot, with no directional travel across the row.
- Do not show literal foot-running, jogging, sprinting, treadmill motion, long steps, pumping arms, speed lines, dust clouds, floor shadows, motion trails, or detached motion effects.
...
Repair attempt 1:
- The previous `running` strip failed QA: running frame 02 is empty or too sparse (23 pixels); running frame 02 is much smaller than the row median (23 vs 25498)
