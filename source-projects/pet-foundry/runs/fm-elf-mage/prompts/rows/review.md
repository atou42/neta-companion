Create a single horizontal sprite strip for the Codex app digital pet `fm-silver-elf-mage` in the state `review`.
...
Animation action: FM loading and tuning loop, the mage studies the tiny staff with narrowed eyes and makes a small tuning gesture.


State-specific requirements:
- Show loading through lean, blink, narrowed eyes, head tilt, or staff position.
- The mood should feel like finding the right radio frequency.
- Do not add magnifying glasses, papers, code, UI, punctuation, symbols, music notes, or other new props unless they already exist in the base pet identity.
