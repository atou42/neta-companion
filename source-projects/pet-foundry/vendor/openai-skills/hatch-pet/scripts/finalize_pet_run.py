...
def validate_image2_provider(job: dict[str, object], *, source: Path, run_dir: Path, job_id: str) -> None:
    provider = job.get("source_provider")
    if not isinstance(provider, dict):
        raise SystemExit(f"job {job_id} is missing source_provider for image2 provenance")
    if provider.get("name") != "image2":
        raise SystemExit(f"job {job_id} has invalid image2 provider name")
    metadata_raw = provider.get("metadata_path")
    expected_metadata_hash = provider.get("metadata_sha256")
    if not isinstance(metadata_raw, str) or not metadata_raw:
        raise SystemExit(f"job {job_id} is missing image2 metadata_path")
    if not isinstance(expected_metadata_hash, str) or not expected_metadata_hash:
        raise SystemExit(f"job {job_id} is missing image2 metadata_sha256")
    metadata_path = Path(metadata_raw).expanduser().resolve()
    if is_relative_to(metadata_path, run_dir):
        raise SystemExit(f"job {job_id} image2 metadata is inside the pet run directory")
    if not metadata_path.is_file():
        raise SystemExit(f"job {job_id} image2 metadata no longer exists: {metadata_path}")
    if file_sha256(metadata_path) != expected_metadata_hash:
        raise SystemExit(f"job {job_id} image2 metadata hash does not match imagegen-jobs.json")
    metadata: dict[str, Any] = json.loads(metadata_path.read_text(encoding="utf-8"))
    files = metadata.get("files")
    request = metadata.get("request")
    endpoint = metadata.get("endpoint")
    if not isinstance(files, list) or not files:
        raise SystemExit(f"job {job_id} image2 metadata is missing files")
    resolved_files = []
    for item in files:
        if not isinstance(item, str):
            raise SystemExit(f"job {job_id} image2 metadata files must be string paths")
        resolved_files.append(resolve_metadata_file_path(item, metadata_path))
    if source.resolve() not in resolved_files:
        raise SystemExit(f"job {job_id} source image is not listed in image2 metadata files")
    if not isinstance(request, dict):
        raise SystemExit(f"job {job_id} image2 metadata is missing request")
    if not isinstance(endpoint, str) or not endpoint:
        raise SystemExit(f"job {job_id} image2 metadata is missing endpoint")
    if request.get("model") != provider.get("model"):
        raise SystemExit(f"job {job_id} image2 model does not match metadata")
    if request.get("size") != provider.get("size"):
        raise SystemExit(f"job {job_id} image2 size does not match metadata")
    prompt = request.get("prompt")
    if not isinstance(prompt, str) or not prompt:
        raise SystemExit(f"job {job_id} image2 metadata request is missing prompt")
    expected_prompt_hash = provider.get("prompt_sha256")
    if not isinstance(expected_prompt_hash, str) or not expected_prompt_hash:
        raise SystemExit(f"job {job_id} is missing image2 prompt_sha256")
    prompt_hash = hashlib.sha256(prompt.encode("utf-8")).hexdigest()
    if prompt_hash != expected_prompt_hash:
        raise SystemExit(f"job {job_id} image2 prompt hash does not match metadata")
    if provider.get("endpoint") != endpoint:
        raise SystemExit(f"job {job_id} image2 endpoint does not match metadata")
...
def validate_completed_job_source(
    ...
    *,
    ...
    if job.get("source_provenance") == "image2":
        ...
        validate_image2_provider(job, source=source, run_dir=run_dir, job_id=job_id)
        validate_hash(job, source=source, output=output, job_id=job_id)
        return

    if job.get("source_provenance") != "built-in-imagegen":
        raise SystemExit(
            f"job {job_id} was not recorded as an accepted provider output; "
            "use record_imagegen_result.py with built-in $imagegen or --provider image2"
        )
    if is_relative_to(source, run_dir):
        raise SystemExit(
            f"job {job_id} source image is inside the pet run directory; "
