Create a single horizontal sprite strip for the Codex app digital pet `fm-silver-elf-mage` in the state `waiting`.
...
Animation action: FM paused loop, the mage holds a small book or staff close, stands still, blinks, and looks patiently at the listener.
...
Repair attempt 1:
- The previous `waiting` strip failed QA: waiting frame 02 is empty or too sparse (20 pixels); waiting frame 02 is much smaller than the row median (20 vs 23657)
