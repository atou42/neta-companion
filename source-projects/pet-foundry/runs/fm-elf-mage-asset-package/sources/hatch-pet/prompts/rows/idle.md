Create a single horizontal sprite strip for the Codex app digital pet `fm-silver-elf-mage` in the state `idle`.
...
Repair attempt 1:
- The previous `idle` strip failed QA: idle frame 04 is empty or too sparse (7 pixels); idle frame 04 is much smaller than the row median (7 vs 23928)
...
- Do not redesign the pet. Keep the exact same head shape, face design, markings, body proportions, palette, outline weight, materials, and props as the approved base pet.
- If the contact sheet shows identity drift, repair only this row while preserving the canonical base identity.
- CRITICAL extraction requirement: the six poses must be six isolated sprite components separated by at least 40 pixels of uninterrupted pure #00FF00 background.
- CRITICAL background requirement: the whole background must be one perfectly flat #00FF00 color, with no gradient, glow, vignette, soft shadow, antialias haze, texture, or lighting.
- No hair, ears, cloak, outlines, staff, or stray pixels may touch another frame's character.
