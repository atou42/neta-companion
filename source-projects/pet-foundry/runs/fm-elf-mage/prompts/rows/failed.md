Create a single horizontal sprite strip for the Codex app digital pet `fm-silver-elf-mage` in the state `failed`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: legally distinct original elf mage, not an exact copyrighted character; silver hair, green eyes, white and gold fantasy robe, tiny staff, calm ancient-mage mood, compact chibi companion for FM playback states.
...
Animation action: FM signal-lost reaction, the mage slumps slightly, closes eyes, and lowers the staff as if the station disappeared.


State-specific requirements:
- Show signal loss through a front-facing slumped pose, drooping ears/limbs, closed or sad eyes, and a lowered staff.
- Every one of the 8 frames must contain one complete full-body front-facing mage pose. Do not leave any requested frame empty.
- Keep the face and robe visible; do not turn the character fully backward.
...
Repair attempt 1:
- The previous `failed` strip failed QA: failed frame 02 is much smaller than the row median (1053 vs 21975); failed frame 06 is much smaller than the row median (842 vs 21975)
...
- CRITICAL: all 8 failed frames must be substantial full-body poses with visible face, robe, ears, hair, and lowered staff.
- No tiny detached head, leaf, sparkle, or partial fragment may be used as a frame.
